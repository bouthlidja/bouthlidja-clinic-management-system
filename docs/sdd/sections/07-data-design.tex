...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{l l l X}
		...
		PWD\_LST\_UPDT& TIMESTAMP     & \makecell[l]{DEFAULT \\ CURRENT\_TIMESTAMP} & Timestamp of the last password update (for 90-day rotation policy). \\
		FLD\_LGN\_ATTMPTS & INT       & DEFAULT 0                   & Counter for failed login attempts (triggers lockout after 5). \\
		IS\_LCKD      & BOOLEAN       & DEFAULT FALSE               & Indicates if the account is currently locked due to failed attempts. \\
		\bottomrule
	\end{tabularx}
\end{table}
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{l l l X}
		...
		BILL\_PAY\_STS & VARCHAR(30) & NOT NULL & Current payment lifecycle status. Must support: \texttt{'Unpaid'}, \texttt{'Processing'}, \texttt{'Paid'}, \texttt{'Partial'}, and \texttt{'Requires\_Reconciliation'}. \\
...
The Payment entity records the actual monetary transactions associated with a bill. It tracks the amount paid, the date of transaction, and the method used (e.g., Cash, Card). To prevent concurrency desynchronization (split-brain scenarios) with external financial networks, this entity securely stores unique transaction payload identifiers for background system audits.
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{l l l X}
		...
		PAY\_MTHD & VARCHAR(30) & NOT NULL & Payment method used (e.g., \texttt{'CASH'}, \texttt{'SATIM'}, \texttt{'BARIDIMOB'}, \texttt{'INSURANCE'}). \\
		PAY\_EXT\_REF & VARCHAR(100) & UNIQUE, NULL & Unique transaction reference ID returned from the external payment gateway provider (TBD-01) used for background automated reconciliation. \\
