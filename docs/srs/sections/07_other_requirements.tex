...
\newpage
\paragraph{7.2.2.4 Patient Billing and Payment Activity Diagram}
...
\textbf{Description:} This appendix lists the functional and technical requirements that are currently identified but require further analysis, external integration details, or administrative decisions before final implementation.
\begin{table}[H]
	\centering
	\renewcommand{\arraystretch}{1.4}
	\begin{tabularx}{\textwidth}{l X X}
		\toprule
		\textbf{ID} & \textbf{Item / Requirement} & \textbf{Reason for TBD Status} \\
		\midrule
		TBD-01 & Payment Gateway Integration & Awaiting specific API documentation from local Algerian banks (e.g., SATIM) or postal services (CCP/Baridimob). \\ \addlinespace
		TBD-02 & SMS Notification Provider & Choosing between local SMS gateway providers for appointment reminders based on cost and API reliability. \\ \addlinespace
		TBD-03 & Biometric Authentication & Evaluating the feasibility of using fingerprint scanners for staff login vs. traditional credentials. \\ \addlinespace
		TBD-04 & Data Migration Strategy & Decisions needed on the format of legacy patient data (if any) to be imported into the new MySQL/Oracle database. \\ \addlinespace
		TBD-05 & Cloud Hosting Provider & Comparing local hosting regulations for medical data vs. international providers (Azure/AWS). \\
		\bottomrule
	\end{tabularx}
\end{table}